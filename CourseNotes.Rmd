...
```
**Important**
When you run that line of code, the `dplyr` package in tidyverse executes the filtering operation and returns a new data frame. If you want to save the result, you’ll need to use the assignment operator, `<-`:, the initial object would be unchanged, unless you overwrite it.
...
heights %>% 
  ...
  arrange(-heightcm)
```
Again, if you do not use the assignment operator, the new variable would not be added to the dataframe object. So to actually make a change to our dataset object, we need to write: 
